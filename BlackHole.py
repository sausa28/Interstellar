...
def loadCreateMaps(vars1d_desired, vars2d_desired):
	...
	if existance[0] == True and existance[1] == True: #both already exist
	...
	elif existance[0] == True and existance[1] ==  False:  #only 1d map exists, need to create 2d map
	...
	elif existance[0] == False and existance[1] == False: #neither map exists, need to create both
...
def plot_multiple_paths(vars1d):
	...
	for i in range(len(input_phis)):
		...
		if HitsBlackHole == True: #if a light ray ends in the black hole, color it black, and set it to be on top
